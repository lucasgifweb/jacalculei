...
class UniteCreatorElementorWidget extends Widget_Base {
    ...
    protected function addAdvancedSectionControls(){
    	...
    	$this->start_controls_section("unlimited_advanced_features", array(
    		"label"=>__("Unlimited Elements Features", 'unlimited_elements'),
    		"tab"=>"advanced"));
		...
		$this->add_control(
			...
			array(
				...
				'return_value' => 'true',
				'default' => '',
				'description'=>__('Show widget data for debugging purposes. Please remove this option when you releasing the widget', 'unlimited_elements')
			)
		);    	
   ...
   protected function ucRegisterControls_addon(){
         ...
         $arrCatsAndParams = $this->sortParamsByCats($arrCats, $params);
         ...
         foreach($arrCatsAndParams as $catID => $arrCat){
         	...
         	$this->start_controls_section($catID, $arrSectionOptions);
	          ...
	          if($isGeneralSection && $this->isConsolidated == false)
