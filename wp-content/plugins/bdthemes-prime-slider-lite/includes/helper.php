...
function prime_slider_et_options() {
    ...
    $data = get_transient( 'ps_elementor_template' );
    ...
    if ( false === $data ) {
        ...
        if ( empty( $templates ) ) {
            $template_options = [ '0' => __( 'You Haven’t Saved Templates Yet.', 'bdthemes-prime-slider' ) ];
        } else {
        ...
        set_transient( 'ps_elementor_template', $template_options, 120 );

        return get_transient( 'ps_elementor_template' );
...
function prime_slider_custom_excerpt($limit=50, $trail = '...') {
